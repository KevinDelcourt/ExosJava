package interactionJeu;

import primitiveLanguage.Jeu;
...
public class Interaction {
	...
	private void placerBateau(int numJoueur, String nomBateau, int tailleBateau) {
		...
		while (!place) {
			...
			System.out.println("O� souhaitez-vous positionner votre "
					+ nomBateau + "?");
			System.out.println("Donnez les coordonn�es de la premi�re case : ");
	...
	private void affichageGrilleCoteJoueur(int joueur) {
		...
		for (char i = 'A'; i < 'K'; i++) {
			for (int j = 1; j < 11; j++) {
				System.out.print(" "
						+ jeu.affichageElementGrilleCoteJoueur(joueur, i, j)
						+ " ");
			}
			System.out.println();
		}
	...
	private void affichageGrilleCoteAdversaire(int joueur) {
		...
		for (char i = 'A'; i < 'K'; i++) {
			for (int j = 1; j < 11; j++) {
				System.out
						.print(" "
								+ jeu.affichageElementGrilleCoteAdversaire(
										joueur, i, j) + " ");
			}
			System.out.println();
		}
